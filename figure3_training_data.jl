...
for id in FreeConvection.SIMULATION_IDS
    ...
    lines!(ax, T_n, zc, color=simulation_color(id); linestyle)
end
...
for id in FreeConvection.SIMULATION_IDS
    ...
    linestyle = 1 <= id <= 9 ? :solid : :dash
    lines!(ax, wT_n, zf, color=simulation_color(id), linestyle=linestyle)
end
...
entries = [LineElement(color=simulation_color(id), linestyle = id == 1 ? :solid : :dash) for id in entry_ids]
